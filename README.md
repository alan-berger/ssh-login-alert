...
```bash
_ssh_auth="${SSH_AUTH_INFO_0%% *}"; [ -z "$_ssh_auth" ] && _ssh_auth="unknown"
...
- `$SSH_AUTH_INFO_0` is set by OpenSSH 7.8+ and contains the authentication method (e.g. `publickey`). On older systems it will fall back to `unknown`.
...
- OpenSSH 7.8+ for authentication method detection (older versions will log `unknown`)
...
Builds a proper MIME email message with correctly formatted Subject, From, and To headers, then branches on the `method` setting in the config:
...
- **smtp**: opens a connection to the configured SMTP server, optionally upgrades to TLS via STARTTLS, authenticates, and relays the message.
